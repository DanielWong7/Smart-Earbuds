    ...
    def run(self):
        ...
        try:
            ...
            ************************************************************
            ...
            ## Object definition to be used by Emulator
            '''
            ***************
            IMPORTANT DISCLAIMER!
            Ideally we would be using a phone app as an output device
            However, due to this design project being done through an emulator we could not make a GUI/phone app
            To mimic this app we used a buzzer and email to represent the notifications in a phone app
            We also used buttons an inputs/outputs through the python shell to mimic a phone touchscreen
            For these reasons our code has implemented a buzzer and push buttons that are not seen on the actual design
            ***************
            '''
            ...
            ## Variable Definition - Can be changed to optimize device performance
            global pauseTime
            pauseTime = 0.1
            unconsciousTime = 45
            minFallDistance = 10
            emulatorError = 30 ## Our distance sensor emulator does not allow for a distance to be measured below 30; this defined variable accounts for this error
            stop = False
            
            '''
            This segment of code continiously checks if the patient has fallen
            It also continiously prints and documents the physical data about the patient
            (distance, speed, average speed, whether they have fallen)
            When the devices senses a fall, the code calls the fallen function to decide what to do next
            This portion of code is also constantly checking if the stop button (button 2) is pressed so that the device can be turned off
            The average speed is reset once every minute to keep the average relevant to the users current activity
            (This prevents sitting for long periods of time from effecting the average speed)
            '''
            while stop == False:
                averageSpeed = 0
                for i in range(1,int(60/pauseTime)+1):
                    if stopButton.is_pressed:
                        ...
                        averageSpeed = updateAverageSpeed(averageSpeed, currentSpeed, i)
                        data = "Raw sensor distance: "+str(sensor.distance())+"\tAverage speed: "+str(round(averageSpeed))+"\tCurrent speed: "+str(round(currentSpeed))+"\tPatient fallen: "+str(checkForFalls(averageSpeed, currentSpeed, sensor.distance(), minFallDistance+emulatorError))+"\tBuzzer: Off\n"
                        recordSensorData(data)
                        print("Raw sensor distance:",sensor.distance(),"\tAverage speed:",round(averageSpeed),"\tCurrent speed:",round(currentSpeed))
                        if checkForFalls(averageSpeed, currentSpeed, sensor.distance(), minFallDistance+emulatorError) == True:
                            data = "Raw sensor distance: "+str(sensor.distance())+"\tAverage speed: "+str(round(averageSpeed))+"\tCurrent speed: "+str(round(currentSpeed))+"\tPatient fallen: "+str(checkForFalls(averageSpeed, currentSpeed, sensor.distance(), minFallDistance+emulatorError))+"\tBuzzer: On\n"
                            recordSensorData(data)
                            fallen(int(unconsciousTime/pauseTime))
            '''
            ************************************************************
...
'''Imports to be used by the functions'''
import datetime ## Used for getting the time the patient fell
import smtplib
from email.mime.text import MIMEText ## The libraries associated with sending emails were imported

def calcCurrentSpeed(): ## This function calculates the current speed using the difference between the intial and final distances and the time elapsed
    initialDistance = sensor.distance()
    time.sleep(pauseTime)
    finalDistance = sensor.distance()
    return abs((finalDistance-initialDistance)/pauseTime) ## The pauseTime variable describes how often the function detects a change in distance

def updateAverageSpeed(avgSpeed, curSpeed, i): ## This function calculates the rolling average by adding new data points to the previously collected data points and then recalculating the average speed
        newAvgSpeed = (avgSpeed*(i-1)+curSpeed)/i
        return newAvgSpeed
        
def checkForFalls(avgSpeed,curSpeed,dist, fallDistance): ## This function determines whether or not the patient has fallen
    if (curSpeed > avgSpeed) and (dist < fallDistance):  ## Initiates the comparison between current and average speeds as well as the distance and the falling distance
        return True
    else:
        return False
'''
The fallen function determines what should be output after a fall is sensed
It activates the buzzer when the patient is on the ground
It prints how much time is left until the emergency contact is notified (based on unconsciousTime variable)
If the patient doesnt get up or activate the failsafe (button 1) before the timer is done, the patients gaurdian will be notified that the patient is seriously injured
If the patient does activate the failsafe (button 1) or gets up, the device checks for injuries
'''
def fallen(koTime):
    buzzer.on() ## If the patient has fallen, a buzzer will be turned on
    print("Patient has fallen")
    print("Buzzer: ON")
    for i in range(koTime): 
        timeLeft = koTime * pauseTime - i * pauseTime
        if timeLeft.is_integer():
            print("Time left until emergency contact is called:",timeLeft)
            print("Button not pressed")
        if ((calcCurrentSpeed() > 0) or failsafe()):
            print("Patient not critically injured")
            print("Buzzer: OFF")
            buzzer.off()
            injuryInspection()
            break
        elif i == koTime-1:
            print("Patient unconscious")
            print("Buzzer: OFF")
            buzzer.off()
            contactEmergencyContact("unconscious")     
'''
This function asks if the patient is injured after a fall and if so,
it notifies the patient's guardian about the fall (in a less urgent way than the serious injury notification)
'''
def injuryInspection():
    print("Checking for Injury")
    var = input("Have you been injured? y/n ")
    if var == "y":
        print("Your guardian/caretaker is being contacted.")
        contactEmergencyContact("injured") ## Email the guardian/caretaker regarding the patient's injury
    elif var == "n":
        return
    
'''
This function emails the patients guardian/emergency contact about a fall
For now it only emails itself (login credentials shown below)
After sending the email, it records the data relevant to the fall (injury status and date/time of fall)
'''
def contactEmergencyContact(status): 
    print("Contacting Emergency Contact...")
    smtp_ssl_host = "smtp.gmail.com" ## This allows to connect with Google's servers
    smtp_ssl_port = 465
    username = "DP3testeremail@gmail.com"
    password = "designproject3"
    from_addr = "DP3testeremail@gmail.com"
    to_addrs = ["DP3testeremail@gmail.com"]
    if status == "injured": ## The if and elif statement allow for two different messages to be sent to the caretaker/guardian depending on the status of the patient
        message = MIMEText("The device has reported an injury. " +
                       "Please take the appropriate protocol to assist the patient's recovery.")
        message["subject"] = "Alert: Patient Status Update"
    elif status == "unconscious":
        message = MIMEText("The device has reported a serious injury. " +
                       "The patient is unconscious or unable to get up.")
        message["subject"] = "WARNING: Important patient status update"
    ...
    server.quit()
    recordFallData(status)

def recordFallData(status): ## This function allows for the data collected to be written to a text file
    file = open("DP3_FallData.txt","a")
    file.write("Patient fell. Status: " + str(status) + ". Date and time: " + str(datetime.datetime.now()) + "\n")
    file.close()

def recordSensorData(data): ## This function allows for the data collected to be written to a text file
    file = open("DP3_SensorData.txt","a")
    file.write(data)
    file.close()
  
def failsafe(): ## This function activates the failsafe feature of the device 
    if button.is_pressed:
          print("Patient activated the failsafe")
